...
\begin{align*}
  \mat{\tau} &= \mat{r} \times \mat{F}
  \intertext{First start by defining the torque on the pendulum bob using the
    force due to gravity.}
  \mat{\tau} &= \mat{l} \times \mat{F}_g
\end{align*}

where $\mat{l}$ is the length vector of the pendulum and $\mat{F}_g$ is the
force due to gravity.
...
\begin{equation*}
  \mat{L} = \mat{r} \times \mat{p} =
    m\mat{r} \times (\mat{\omega} \times \mat{r})
\end{equation*}

Again just consider the magnitude of the angular momentum.
\begin{align*}
  \lvert\mat{L}\rvert &= mr^2\omega \\
  \lvert\mat{L}\rvert &= ml^2 \frac{d\theta}{dt} \\
  \frac{d}{dt}\lvert\mat{L}\rvert &= ml^2 \frac{d^2\theta}{dt^2}
  \intertext{According to $\tau = \frac{d\mat{L}}{dt}$, we can just compare the
    magnitudes.}
...
\begin{align}
  \Delta U &= mgh \nonumber
  \intertext{The change in kinetic energy (body started from rest) is given by}
  \Delta K &= \frac{1}{2}mv^2 \nonumber
  \intertext{Since no energy is lost, the gain in one must be equal to the loss
    in the other}
  \frac{1}{2}mv^2 &= mgh \nonumber
  \intertext{The change in velocity for a given change in height can be
  expressed as}
  v &= \sqrt{2gh} \nonumber
  \intertext{Using equation \eqref{eq:arc_length}, this equation can be
    rewritten in terms of $\frac{d\theta}{dt}$.}
...
\begin{align}
  y_0 &= l\cos\theta_0 \nonumber
  \intertext{Similarly for $y_1$, we have}
  y_1 &= l\cos\theta \nonumber
  \intertext{Then $h$ is the difference of the two}
  h &= l(\cos\theta - \cos\theta_0) \nonumber
  \intertext{Substituting this into equation \eqref{eq:energy_dtheta} gives}
  \frac{d\theta}{dt} &= \sqrt{\frac{2g}{l}(\cos\theta - \cos\theta_0)} \nonumber
  \intertext{This equation is known as the first integral of motion. It gives
    the velocity in terms of the location and includes an integration constant
    related to the initial displacement ($\theta_0$). We can differentiate by
    applying the chain rule with respect to time. Doing so gives the
    acceleration.}
