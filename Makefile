...
ifeq ($(OS),Windows_NT)
	...
	VENV_PYTHON := ./build/venv/bin/python
...
build/venv.stamp:
	...
	$(VENV_PIP) install frccontrol==2026.12 sleipnirgroup-jormungandr==0.6.3 ruff==0.15.18 qrcode requests
	@touch $@
...
format: build/venv.stamp
	...
	# Format .py
	find . -type f -name '*.py' -not -path './build/*' -print0 | xargs -0 $(abspath ./build/venv/bin/python) -m ruff format -q
...
lint_py: format build/venv.stamp
	find . -type f -name '*.py' -not -path './build/*' -print0 | xargs -0 $(abspath ./build/venv/bin/python) -m ruff check --fix -q
