...
format: build/venv.stamp
	...
	# Format .py
	find . -type f -name '*.py' -not -path './build/*' -print0 | xargs -0 $(abspath ./build/venv/bin/python3) -m ruff format -q
...
lint_py: format build/venv.stamp
	find . -type f -name '*.py' -not -path './build/*' -print0 | xargs -0 $(abspath ./build/venv/bin/python3) -m ruff check --fix -q
