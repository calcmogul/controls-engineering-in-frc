...
Thanks to Austin Schuh for providing the state-space model examples.
...
\section{Methods of integral control}
...
\begin{equation*}
  \mtx{u} = \mtx{K} \left(\mtx{r} - \mtx{x}\right) - u_{error}
\end{equation*}
\\
...
\end{equation*}
\\
where a $1$ is augmented if $u_{error}$ corresponds to an existing input and a
$0$ is augmented otherwise. $\mtx{r}$ is augmented with a zero for the goal
$u_{error}$ term.
