...
Snippet \ref{lst:linear_mpc} shows a linear MPC implementation in Python.
\begin{coderemote}{Python}{snippets/linear_mpc.py}
  \caption{Linear MPC implementation in Python}
...
Snippet \ref{lst:nonlinear_mpc} shows a nonlinear MPC implementation in Python.
\begin{coderemote}{Python}{snippets/nonlinear_mpc.py}
  \caption{Nonlinear MPC implementation in Python}
...
Using the previous timestep's optimal trajectory to warm start the current solve
can drastically speed up convergence.

We recommend setting a timeout after which the solver returns its solution so
far. Applying the control input from a partially optimized trajectory is better
than overrunning the scheduled controller period.
