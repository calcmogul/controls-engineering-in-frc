...
\noindent and $\hat{x} = z_1$. That is, the weight is on the observations and
the estimate is equal to the measurement. \\
...
\noindent This can be converted to the following state-space \gls{model}.
...
\noindent where the Gaussian random variable $w_k$ has zero mean and variance
