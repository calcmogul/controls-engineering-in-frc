...
\begin{remark}
  Chapters including and after this use Python Control to demonstrate the
  concepts discussed and perform the complex math required. See appendix
...
\noindent the canonical \gls{realization} of it that satisfies equation
(\ref{eq:ctrl_rank}) is
...
\noindent The canonical \gls{realization} of the \gls{system} in equation
...
\noindent Plugging in equation (\ref{eq:ssz_ctrl_y}) gives
...
\noindent Subtracting this from equation (\ref{eq:ssz_ctrl_x}) gives
...
\noindent where $\mtx{\nu}_k$ is the measurement noise. As $\mtx{L}$ increases,
the measurement noise is amplified. \\
