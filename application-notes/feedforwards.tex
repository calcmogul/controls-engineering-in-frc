...
Now, we'll do the same thing for the discrete system. To find the control input
required at steady-state, set equation (\ref{eq:ssz_ctrl_x}) to zero.
...
allows using specific actuators to maintain the reference at steady-state. The
two-state feedforward doesn't support this, but can be used for reference
tracking as well with the same tuning parameters as LQR design. Figure
\ref{fig:case_study_ff1} shows both types of feedforwards applied to a
...
because the $\mtx{R}$ matrix penalized actuation effort. If the $\mtx{R}$ cost
matrix is removed from the two-state feedforward calculation, the reference
tracking is much better (see figure \ref{fig:case_study_ff2}).
