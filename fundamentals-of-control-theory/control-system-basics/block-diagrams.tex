...
\begin{bookfigure}
  \begin{tikzpicture}[auto, >=latex']
    ...
    \node [right=of sum, align=center] (P1) {open-loop\\path};
    \node [right=of P1] (output) {output};
    \node [below=of P1, align=center] (P2) {feedback\\path};

    % Connect the nodes
    \draw [arrow] (input) -- node[pos=0.85] {$+$} (sum);
    \draw [arrow] (sum) -- (P1) -- node[name=y] {} (output);
    \draw [arrow] (y) |- (P2) -| node[pos=0.97, right] {$\mp$} (sum);
  \end{tikzpicture}
