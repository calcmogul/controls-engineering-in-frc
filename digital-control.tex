...
frequency of the system dynamics being above the Nyquist frequency\footnote{To
completely reconstruct a signal, it must be sampled at twice the highest
frequency it contains, called the Nyquist rate. The highest frequency a given
sample rate can capture is called the Nyquist frequency, which is half the
sample rate. Frequencies above this threshold are folded down across it. The
higher frequency and the folded down lower frequency are said to alias each
other.}. The discretized signal doesn't have enough samples to reconstruct the
