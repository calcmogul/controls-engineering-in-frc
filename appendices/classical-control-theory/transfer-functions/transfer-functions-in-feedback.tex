...
velocity and the other corresponds to current.

Compare the step response of this \gls{system} (figure
\ref{fig:tf_cim_second_order_step}) with the step response of this \gls{system}
with $L$ set to zero (figure \ref{fig:tf_cim_first_order_step}). For small
values of $K$, both \glspl{system} are stable and have nearly indistinguishable
\glspl{step response} on a long timescale. The high frequency dynamics only
cause oscillation for large values of $K$ that induce fast
\glspl{system response}. In other words, the \glspl{system response} of the
second-order model and its first-order approximation are similar for low
frequency operating regimes.
\begin{bookfigure}
  \begin{minisvg}{2}{build/figs/cim_second_order_step}
    \caption{Second-order CIM motor model step response ($L = 230$ μH)}
    \label{fig:tf_cim_second_order_step}
  \end{minisvg}
  \hfill
  \begin{minisvg}{2}{build/figs/cim_first_order_step}
    \caption{First-order CIM motor model step response ($L = 0$ μH)}
    \label{fig:tf_cim_first_order_step}
  \end{minisvg}
\end{bookfigure}
